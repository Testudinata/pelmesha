...
def mspeaks_opt(X, Y,spectra_ind, fwhhfilter=0,oversegmentationfilter=0,heightfilter=0,rel_heightfilter=0,peaklocation=1,noise_func = np.std ,noise_est_iterations = 3, SNR_threshold = 3.5,print_queue = None):
    ...
    valley_dots = np.concatenate((np.where(np.diff(Y) != 0)[0], [xsize-1]))    
    ...
    return np.column_stack(([spectra_ind]*signal_num,pkX, val_max, pkA,(val_max-mean_noise)/noise,X[left_min+1], X[right_min-1],lfwhh, rfwhh,[noise]*signal_num,[mean_noise]*signal_num))
...
def mspeaks_arrayopt(X, Y_array,spectra_ind, fwhhfilter=0,oversegmentationfilter=0,heightfilter=0,rel_heightfilter=0,peaklocation=1,noise_func = np.std ,noise_est_iterations = 3, SNR_threshold = 3.5,print_queue = None):
    ...
    for ns,ind in enumerate(spectra_ind):
        ...
        valley_dots = np.concatenate((np.where(valley_bool[ns,:])[0], [xsize-1]))    
        loc_min = np.diff(Y[valley_dots])
        loc_min = (np.array([True,*(loc_min < 0)])) & np.array(([*(loc_min > 0),True]))
        left_min = np.concatenate([[-1],valley_dots[:-1]])[loc_min][:-1] + 1
        right_min = valley_dots[loc_min][1:]
    ...
    # Compute max and min for every peak
        ...
        peaklists[ns] = np.column_stack(([ind]*signal_num,pkX, val_max, pkA,(val_max-mean_noise)/noise,X[left_min+1], X[right_min-1],lfwhh, rfwhh,[noise]*signal_num,[mean_noise]*signal_num))  
    #print_queue.put(f"{[spectra_ind]*sum(k)},\n{pkX[k]}, \n{val_max[k]},\n {pkA},\n{val_max[k]/noise,lfwhh[k]},\n {rfwhh[k]},\n{X[left_min[k]]}, \n{X[right_min[k]]},\n{noise},\n{mean_noise}")
    return np.vstack(tuple(peaklists.values()))

def peaks_prop_array_inline(X, Y_array,spectra_inds, fwhhfilter=None,oversegmentationfilter=None,heightfilter=None,rel_heightfilter=None,peaklocation=1,noise_func = np.std ,noise_est_iterations = 3, SNR_threshold = None, Calc_peak_area = True):
    """
    Общее описание
    ----
    Функция для получения пиклиста с характеристиками пиков из одного спектра. Оптимизирована под использование в получении пиклистов в дисконтинуальных данных без resample'а данных.
    ...
    # Robust valley finding
    valley_bool = np.diff(Y_array) != 0
    for ns,spectra_ind in enumerate(spectra_inds):
        Y = Y_array[ns,:]
        # Robust valley finding
        valley_dots = np.concatenate((np.where(valley_bool[ns,:])[0], [xsize-1]))    
        loc_min = np.diff(Y[valley_dots])
        ...
        peaklists[ns] = np.column_stack(([spectra_ind]*signal_num,pkX, val_max, *(props[key] for key in peaks_prop_array_inline.headers)))
    return np.vstack(tuple(peaklists.values()))
...
def peaks_prop(X,Y,spectra_ind,fwhhfilter,oversegmentationfilter,heightfilter,rel_heightfilter,peaklocation,noise_func,noise_est_iterations, SNR_threshold, Calc_peak_area):
    """
    Общее описание
    ----
    ...
    props["PextL"] = X[left_min+1] 
    props["PextR"] = X[right_min-1] 
    return np.column_stack(([spectra_ind]*signal_num,pkX, val_max, *(props[key] for key in peaks_prop.headers)))
